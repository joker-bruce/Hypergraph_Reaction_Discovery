...
from dgllife.utils.mol_to_graph import mol_to_complete_graph,mol_to_bigraph
...
def reshape_tensor(tensor, n):
    m_squared, channels = tensor.shape
    m = int(m_squared**0.5)
    assert m * m == m_squared, "Input tensor's first dimension must be a perfect square"
    assert channels == 5, "Input tensor's second dimension must be 5"

    if m < n:
        # biased with -1e4
        new_tensor = torch.full((n * n, channels), -1e4)
        new_tensor[:m * m] = tensor
    else:
        # Truncate
        new_tensor = tensor[:n * n]

    return new_tensor

def smiles_to_pretrained_gnn_data(reactant_smiles):
    # add atom mapping to the reactants and convert it into a complete graph and bigraph
    mol = Chem.MolFromSmiles(reactant_smiles)
    for idx, atom in enumerate(mol.GetAtoms()):
        atom.SetAtomMapNum(idx + 1)
    reactant_mapping_smiles = Chem.MolToSmiles(mol)
    complete_graph = mol_to_complete_graph(
                    mol, add_self_loop=True, canonical_atom_order=False)
    atom_pair_features = default_atom_pair_featurizer(reactant_mapping_smiles)
    mol_graph = mol_to_bigraph(mol, node_featurizer=default_node_featurizer_center,
                                edge_featurizer=default_edge_featurizer_center,
                                canonical_atom_order=False)
    #mol_graph = #dgl.batch(mol_graph)
    mol_graph.set_n_initializer(dgl.init.zero_initializer)
    mol_graph.set_e_initializer(dgl.init.zero_initializer)

    #complete_graph = dgl.batch(complete_graph)
    complete_graph.set_n_initializer(dgl.init.zero_initializer)
    complete_graph.set_e_initializer(dgl.init.zero_initializer)
    complete_graph.edata['feats'] = atom_pair_features

    return mol_graph, complete_graph
...
def parse_args():
    ...
    parser.add_argument("--datapoints", type=int, default=100, help='# of datapoints')
    parser.add_argument("--datapath", type=str, default='./grant_uspto_small.csv', help='datapath to access the dataset')
    parser.add_argument("--output_dir", type=str, default='./data', help='datapath to storage the output .pt file')
    parser.add_argument("--output_filename", type=str, default='hyperedge', help='output .pt filename (with suffix datapoints and features added later)')
    parser.add_argument('--ns_ratio', type=float, default=1, help='negativ sampling ratio (neg:ground truth)')
    parser.add_argument('--rxn_type', type=int, default=672, help='reaction type to consider')
