...
df = pd.read_csv("./train_with_label.csv")
neg_df = pd.read_csv('./rcns_with_label.csv')
...
def prep_data(datapoints, data_path, df, neg_df, ns_algo, ns_ratio, output_dir, output_filename):
   ...
   #             neg_smiles_list.append(neg_smiles)
    ...
    neg_smiles_list_len = len(neg_df)
...
# Check if the directory exists
    ...
    labels = df_hyper['rxn_type'].tolist()
    ...
    cns_neg_hyperedges, \
    hyperedge_set, \
    neg_hyperedge_set = process_result(results, labels, output_dir, output_filename, neg_smiles_list_len)   
...
def process_result(results, labels,  output_dir, output_filename, neg_smiles_list_len):
    hyperedge = results
    ...
    hyperedge_set_labels, neg_hyperedge_set_labels = labels[:-neg_smiles_list_len], labels[-neg_smiles_list_len:]
   ...
   # print('hyperedge_set',hyperedge_set)
    ...
    prepare_training(list(hyperedge_set), list(neg_hyperedge_set), hyperedge_set_labels, neg_hyperedge_set_labels, mns_neg_hyperedges, sns_neg_hyperedges, cns_neg_hyperedges, output_dir, output_filename)
    #print(hyperedge_set)
...
def prepare_training(HE_set, HE_neg_set, hyperedge_set_labels, neg_hyperedge_set_labels, mns_neg, sns_neg, cns_neg, output_dir, output_filename):
    HE = HE_set
    target_type = 586
    HE = [he for he, label in zip(HE_set, hyperedge_set_labels) if label == target_type]
    HE_test = [he for he, label in zip(HE_set, hyperedge_set_labels) if label != target_type]
    #start from here
    ...
    for split in range(1):
        ...
        ground_num = int(0.8*len(HE))## all of the HE with label 672 becomes the training postiive
        ground_neg_num = int(0.6*len(HE_neg)) ## keep the other setting unchanged, but the training set
        ...
        for idx in total_idx :
            if idx in ground_idx:
                ...
                pred_data.append(list(HE[idx]))## rest of data for train_only and test_only
        ...
        test_num = len(HE_test)#len(pred_data) - (valid_only_num + train_only_num) 
        ...
        test_data = HE_test#pred_data[-test_num:]
...
if __name__ == "__main__":
    ...
    data_no_feat, data_ecfp4_feat, data_ecfp6_feat, mns_neg_hyperedges, sns_neg_hyperedges, cns_neg_hyperedges, hyperedge_set, neg_hyperedge_set = prep_data(args.datapoints, args.datapath, df, neg_df, 'mns', args.ns_ratio, args.output_dir, args.output_filename)
    end_time = time.time()
